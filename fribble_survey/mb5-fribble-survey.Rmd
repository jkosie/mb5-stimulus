...
base_data %>% 
  ...
  labs(x = "Base Shape", y = "Average Rating")

base_data %>% 
  ggplot(aes(x = item, y = rating, fill = item)) +
  geom_boxplot() +
  geom_point(aes(x = reorder(item, -avg_rating), y = rating), position=position_jitter(width =.15), size = 2, shape = 1) +
  theme(legend.position = "none", axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  labs(x = "Base Shape", y = "Average Rating")
...
by_item_data %>% 
  ...
  geom_bar(stat = "identity") +
  theme(legend.position = "none", axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  labs(x = "Entire Fribble (base + appendages)", y = "Average Rating")
